...
class m_SetByArrayList<E> implements Set<E> {
    ...
    @Override
    public int size() {
        Iterator<E> iter = m_arraylist.iterator();
        int i = 0;
        while(iter.hasNext()){
            iter.next();
            i++;
    ...
    */
    ...
    public boolean contains(Object o) {
        ...
        int length = m_arraylist.size();
        for(int i = 0; i < length; i++){
            if(iter.next().equals(object_in_class_E)){
                return true;
            }
        }
        return false;
